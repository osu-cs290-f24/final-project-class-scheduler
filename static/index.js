...
function checkOverlapTime(newDays, newFromTime, newToTime){
    ...
    allClassElements.forEach(function(classElement) {
        ...
        for(var i = 0; i < newDays.length; i++){
            for(var j = 0; i < days.length; j++){
                if(newDays[i] == days[j]){
                    if((newFromTime <= classData.dataset.fromTime && newToTime > classData.dataset.fromTime) 
                        || (classData.dataset.fromTime <= newFromTime && classData.dataset.toTime> newFromTime)){
                        alert("Colliding!");
                        return true;
                    }
                }
...
function handleModalAcceptClick() {
    ...
    //var collidingTimes = checkOverlapTime(newDays, from);
...
function centerClassText(classElement){
